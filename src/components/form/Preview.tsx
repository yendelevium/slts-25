...
export default function Preview() {
  ...
  // Event component
  const { group, devotionalSinging, individualChoice1, individualChoice2, participateInQuizDrawing, participateInGroupEvent } = formData;
    ...
    <div className="space-y-8">
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-y-3">
          ...
          <Info label="Year of Joining" value={formData.yearOfJoining.toString()} />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-y-3">
          {group === "1" && (
            <>
              {/* devo only */}
              {devotionalSinging && !individualChoice1 && !individualChoice2 && (
                <Info label="Devotional Singing" value={devotionalSinging.toString()} />
              )}
              ...
              {devotionalSinging && individualChoice1 && !individualChoice2 && (
                <>
                  <Info label="Devotional Singing" value={devotionalSinging.toString()} />
                  <Info label="Individual Event 1" value={individualChoice1.toString()} />
                </>
              )}
              ...
              {!devotionalSinging && individualChoice1 && individualChoice2 && (
                <>
                  <Info label="Individual Event 1" value={individualChoice1.toString()} />
                  <Info label="Individual Event 2" value={individualChoice2.toString()} />
                </>
              )}
            </>
          )}
          ...
          {(group === "2" || group === "3") && (
            <>
              ...
              {(participateInQuizDrawing === "yes" ||
                ...
                <>
                  {/* EVENT 1 → ALWAYS quiz */}
                  <Info label="Individual Event 1" value={participateInQuizDrawing.toString()} />
          ...
          {group === "4" && (
            <>
              ...
              {participateInQuizDrawing === "none" && (
                <CardDescription className="col-span-2">Not Participating in Quiz</CardDescription>
              )}
            </>
          )}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-y-3">
          ...
          <Info label="Arrival Time" value={formData.arrivalTime.toString() || "—"} />
          <Info label="Need Pickup" value={formData.needPickup.toString() } />

          {formData.needPickup === "yes" && (
            <>
              <Info label="Arrival Mode" value={formData.arrivalMode.toString()  || "—"} />
              <Info label="Pickup Point" value={formData.pickupPoint.toString()  || "—"} />
            </>
          )}
          ...
          <Info label="Departure Time" value={formData.departureTime.toString()  || "—"} />
          ...
          {formData.needDrop === "yes" && (
            <>
              ...
              />
              <Info label="Drop Point" value={formData.dropPoint.toString()  || "—"} />
            </>
          )}
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-y-3">
              ...
              <Info label="Point of Contact" value={formData.pocName.toString()} />
              <Info label="Relation" value={formData.pocRelation.toString()} />
              <Info label="Gender" value={formData.pocGender.toString()} />
              <Info label="Contact Phone" value={formData.pocPhone.toString()} />
      ...
      {showAccom && (
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-y-3">
            <Info label="Check-In" value={`${fmt(formData.checkInDate)} ${formData.checkInTime || ""}`} />
            <Info label="Check-Out" value={`${fmt(formData.checkOutDate)} ${formData.checkOutTime || ""}`} />
